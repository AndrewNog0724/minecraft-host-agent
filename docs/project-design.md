...
| 安装路径 | **全平台统一：`<数据目录>/runtime/jdk-<major>/<完整版本号>/`（受管目录；v0.12.4 撤销 D21——Windows 不再装 `C:\Program Files\Java\`，受管 JRE 常驻数据目录，`%APPDATA%\mcha\runtime\`）** | **v0.12.4 实测复盘**：Program Files 方案引入 UAC 弹窗、迁移、旗标、降级留痕一整条复杂度链，而目标用户（不懂技术的玩家）对 UAC 弹窗只有恐惧和乱点；数据目录用户必然可写、卸载=删目录、与 mcha 数据同生共死——位置自解释，无需提权 |
| Windows 写入策略 | **无提权（v0.12.4）**：受管目录在数据目录下，普通权限直接解压落盘，全程不弹 UAC；D27 提权统一入口随之退役（当前无任何需要提权的操作） | 复杂度整链移除：迁移逻辑、migrate-declined 旗标、`run_elevated_ps`、UAC 预告全部删除 |
| 选择顺序 | ① 系统 PATH 已有匹配版本 → 用系统的；② 受管目录（数据目录 runtime/）已有 → 复用；③ 下载安装（Adoptium 压缩包，解压后 `<release>/<release>-jre/bin/java` 双层嵌套已处理；每次任务仍先走 ①②，"不缺就不装"）（**v0.12.4：原 ② Program Files 扫描随 D21 撤销删除**） | 尊重玩家已有环境；重复开服零下载 |
...
- 执行环安全边界（v0.12 决议 D26）：本地副作用工具的路径收敛——工作区 + 数据目录受管目录，写入前 canonicalize + `starts_with` 校验，越界一律拒绝；`http_download` 白名单域静默放行、白名单外先 `ask_user` 确认并留痕；LLM 不可达注册表 / 系统 PATH / 防火墙（这些确认保持确定性 CLI 流程，不交给模型）。**（v0.12.4：提权统一入口随 D21 撤销退役，见 D27）**
...
scripts/              # 环境引导（FR-18，决议 D13）：bootstrap-windows.ps1 / bootstrap.sh
```
...
| D2 | Java 供给 | **全自动受管安装，不降级**；设计见 §8.8。**（v0.12.4：受管位置统一为数据目录 `runtime/`，全平台一致、零提权——D21 撤销）** |
...
| D21 | Java 安装位置（v0.11，~~用户硬性要求：统一放 `C:\Program Files\Java\<版本名>\bin\java.exe`~~ **v0.12.4 用户撤销**） | **【已撤销】** 原方案（提权写 Program Files + 数据目录旧 JRE 自动迁移 + 拒绝旗标 + 降级留痕）在 Windows 实机验收中暴露整链复杂度：UAC 弹窗惊吓玩家、迁移静默跳过难排查（v0.12.3 A1）、多一次提权就多一个挂死面。**v0.12.4：受管 JRE 全平台常驻数据目录 `%APPDATA%\mcha\runtime\jdk-<major>\`，普通权限直接解压，无 UAC、无迁移、无旗标** |
...
| D27 | 提权统一入口与挂死防护（v0.12） | **【已退役，v0.12.4】** D21 撤销后项目内不再有任何提权操作（数据目录普通权限可写），`run_elevated_ps` / UAC 预告 / 120s 超时 / 迁移逻辑整体删除；历史上该机制防住的"UAC 未确认永久挂死"问题随提权场景消失而不复存在 |
| D30 | 大文件下载可靠性（v0.12.4，用户实测：spigot jar 下载中被中止、报"网络请求失败"） | 根因：`HttpBase` 全局客户端 120s **总时长**超时对大文件是错误口径——50MB+ 的 jar 在慢速链路上必然被掐断（且三次重试每次都从头再来）。修复：① 下载走**专用客户端**（仅 connect 超时 10s，无总超时）；② 流式读取加**分块空闲超时**（45s 收不到新字节才判死，数据在流动就永不中断）；③ **Range 断点续传**：重试 / 复次调用若发现 `.part` 临时文件，带 `Range: bytes=<已有字节>-` 续传（服务器 206 → 追加并把已有字节补入哈希；200 → 从头重来），重试不再前功尽弃；④ 保留指数退避重试 3 次 + 取消令牌；⑤ 进度事件显示 已下载/总量/实时速度，让用户看得到"在动" |
...
| 2026-08-31 | v0.12.2 | **白名单 uuid 勘误**（Windows 实测：输入了玩家 ID 但进服被拒"not whitelisted"）。根因：`write_configs` 写 whitelist.json 条目只有 `{"name"}` 缺 **`uuid` 字段**——Minecraft 加载白名单时静默丢弃无 uuid 条目，表现为 white-list=true + 名单实际为空、所有人（含服主）被拒。修复：条目补离线 UUID（Mojang 口径：v3，`MD5("OfflinePlayer:<name>")` + RFC 4122 摆位；白名单仅出现在离线/混合场景，离线口径即正确口径）；新增 `md-5` 依赖与结构断言回归测试 |
| 2026-08-31 | v0.12.3 | **Spigot 全流程首次跑通后的细节勘误批量**（实测复盘：开服成功、127.0.0.1:25565 可进服）。① **A1 迁移留痕**：`resolve_java` 返回值增加来源注记（系统/统一根复用/迁移成功/迁移跳过原因——含"此前拒绝迁移，删旗标可重启"指引），必须并入步骤收尾摘要，杜绝静默沿用数据目录旧安装；② **A3 占位 spec_id**：`normalize_draft` 剥离 draft/test/spec/sample 类占位符（实测档案与 MOTD 均变 `draft_001`），回退语义化生成；③ **A4 备份目录**：session-backups 默认从编译期 `CARGO_MANIFEST_DIR`（全局安装后纯属构建机巧合）改数据目录；④ **B1**：PATH 无 java（NotFound）归一为"未检测到"而非 WARN 报错；⑤ **B2**：java 路径分隔符按平台（修 `bin/java.exe` 混合斜杠）；⑥ **A2 部分/B3**：连接卡片内网 IP 自动探测（UDP connect 路由源地址，零发包）、公网 IP 占位符附获取指引、卡片不再整段重复 notes；⑦ **B4**：执行环提示词明确"工具顺序执行，不得向玩家描述并行"。公网 IP 自动检测与樱花frp 编排归入 P1 |
| 2026-08-31 | v0.12.4 | **D21 撤销 + 下载可靠性（D30）**（用户两项调整：① Java 路径保持数据目录 `%APPDATA%\mcha\runtime\` 即可，不再迁 Program Files——"绕开棘手问题"；② spigot jar 下载被中止报"网络请求失败"，不合理）。① 删除整条 Program Files 复杂度链：统一根扫描、JRE 迁移、migrate-declined 旗标、`run_elevated_ps` / UAC 预告 / 120s 超时（D27 退役）——受管 JRE 全平台常驻数据目录，普通权限直接解压，全程零 UAC；A1 来源注记保留（复用/新装仍留痕）。② 下载根因是全局客户端 120s 总时长超时掐断大文件流式传输（重试还每次从头来）；按 D30 改专用下载客户端 + 45s 分块空闲超时 + Range 断点续传 + 速度显示。§8.8/§12/§13 同步更新 |
