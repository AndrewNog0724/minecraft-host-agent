...
**开工前置（先看后动，D141）**：进入本指南后、提问之前，先 `list_dir` 感知
工作区现状——已有 `server/` 目录（或 worlds / mods / logs/latest.log）即说明
工作区已有部署；再查 `~/.mcha/profiles/` 有无部署档案。发现已有部署时，先向
用户陈述现状（版本、端类型、目录），并确认意图：**沿用现有服 / 继续未完成的
部署 / 确认后新开**，不得默认新开。之后才开始下表的信息收集。
